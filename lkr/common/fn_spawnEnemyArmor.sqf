/*
	File: fn_spawnEnemyArmor.sqf
	Author: lukrop

	License: see LICENSE.txt
	Description:
		Spawns a enemy armored vehicle.
...
_unitsList = [(lkr_enemy_inf_units_C call BIS_fnc_selectRandom), (lkr_enemy_inf_units_C call BIS_fnc_selectRandom), (lkr_enemy_inf_units_C call BIS_fnc_selectRandom)];

// spawn the group
_enemyGroup = [_pos, lkr_enemy_side, _unitsList, lkr_enemy_skill_range] call lkr_fnc_spawnGroup;

_veh = createVehicle [lkr_enemy_armor_C call BIS_fnc_selectRandom, _pos, [], 0, "NONE"];

((units _enemyGroup) select 0) moveInDriver _veh;
((units _enemyGroup) select 1) moveInGunner _veh;
((units _enemyGroup) select 2) moveInCommander _veh;
