...
_unitinfo = _this select 1;
	...
	_delay = if (count _unitinfo >= 3) then {_unitinfo select 2}  else {20};
...
if (_inf) then {
	...
	[_spawnname,_side,_delay,_position,_temparray,_infskill,_infgrp,_task] call SLP_fnc_Infantry;
	} else {
		if (_debug==1) then {
...
if (_veh) then {
    if ( count _vehicles >=1) then {
  	[_spawnname,_side,_delay,_position,_temparray,_infskill,_vehskill,_vehgrp,_task,0] call SLP_fnc_vehicle;
	} else {
		if (_debug==1) then {
...
if (_arm) then {
    if ( count _armor >=1) then {
  	[_spawnname,_side,_delay,_position,_temparray,_infskill,_vehskill,_armgrp,_task,1] call SLP_fnc_vehicle;
	} else {
		if (_debug==1) then {
...
sleep 1;
 ...
 if (_air) then {
	...
	[_spawnname,_side,_delay,_position,_temparray,_airskill,_airgrp,_task] call SLP_fnc_air;
	} else {
		if (_debug==1) then {
